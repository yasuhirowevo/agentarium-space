...
- アプリのバージョンを `0.4.0` に更新し、常時注記と親子表示を反映した README の動画・画像を撮り直す。
- 既存の架空ログによる24秒のデモを1920×1080で撮影し、プレビュー画像は23秒時点の完了状態・実行結果が分かる場面とする。
  両 README の動画リンクを揃え、架空データの注記と公開前の検査を維持する。
...
- 選択・hover中のメイン情報は、発話の有無にかかわらず自動注記より先に配置する。
  自動表示の4エージェント枠が埋まっていても、明示的に指定した対象の情報は表示候補に残す。

## 子エージェントの識別と親の表示維持（v2.30）

- Codexの分割ログ `rollout-<日時>-<session ID>_<segment ID>.jsonl` は前側のsession IDで
  識別する。同じsessionの複数ファイルを別エージェントとして表示せず、ファイル名の日時順で
  最新segmentを採用する。通常 / 分割rolloutの公開keyは監視rootとsession IDを基準とし、分割が増えても
  同じorbとして更新する。最新segmentのメタ情報が未完成なら、直前の読み取れるsegmentを
  表示し、完成後に切り替える。起動時も同じ規則を適用する。
  guardianの`session_id`は親を指す場合があるため、子自身の`id`を上書きしない。
  通常ログ・forkのメタデータ照合と読取上限は維持し、履歴全体の再生は行わない。
- 表示対象の子がいる間は、ログで確認できた親と祖先も表示対象に残す。実行中だけでなく、
  完了後60秒の子の表示猶予も含める。子の表示期限やstale上限は延長しない。
- 子であることと、表示中の親へ接続できることを分ける。親が取得できない子も0.6倍のorbと
  小さい名札を維持し、ツリーと集計でも子として扱う。親不在時は単独配置とし、関係線を推測で
  補わない。親が取得できれば既存のSatellite軌道と親子線を使う。
  本節はv2.17 / Satellite仕様の「親不在時は通常orb」を更新する。

## メインエージェントの常時注記（v2.29）

- メインエージェントには、発話に加えて「作業・計画」「直近の実行結果・変更ファイル」
  「モデル・使用状況」をそれぞれ独立した引出線で常時表示する。最大4本とし、取得済みの
  情報がない項目は出さない。sub-agentは発話と補助1行の既存構成を維持する。
  親情報が欠けた子も判別できるよう、既存の内部子判定をsnapshotの`isSubAgent`へ公開する。
- 作業注記は観測されたturn状態、現在のツール、明示的な待機、計画の完了数と現在の
  ステップを示す。実行結果は現在 / 直近のturnに属するコマンドの結果・終了コードと
  変更ファイルを示し、そのturnが続く間は秒数で消さない。過去turnの結果を現在の結果に
  混ぜず、コマンド成功をテスト全体の成功と解釈しない。
- 使用状況は取得済みモデル、推論強度、コンテキスト、出力、ブランチを短く表示する。
  hover / 選択では発話を拡張し、同じメタ情報を別の引出線へ重複表示しない。
- 本文は11pxの正体・行間16px、白の約82%、種別見出しは寒色 / 暖色の既存source色とする。
  発話は通常4行 / 選択6行、メインの各情報注記は最大4行を目安とし、幅は最大240px。
  背景カードは付けず、引出線の明度と注記間の余白を増やす。
- エージェントの配置は維持し、発話を優先したうえで空き領域へ情報注記を追加する。
  メインの常時情報を先に配置し、sub-agentの発話も空いた方向へ配置する。
  orb・名前・セクター見出し・他の注記と重ねず、引出線が別注記の本文を横切らないように
  配置する。狭い画面では情報注記から省略し、方向の安定性と既存の描画制約を維持する。
  自動注記の最大4エージェント枠は維持し、その枠内のメインに複数本を表示する。

## Windowsでのログ追記監視（v2.28）

- Windowsでは、開いたままのJSONLへの追記でファイルサイズだけが増え、更新時刻が変わらない
  場合も読み取りを再開する。Codex / Claudeの両監視で、通常のadd / changeに加え、
  OSからのraw change通知を既存の対象パス検証と差分読込へ渡す。
- 読取対象の階層・拡張子・読取上限を維持する。重複通知は既存のファイル別キューと
  読込位置で吸収し、同じイベントを二重に計上しない。全件の定期再走査は追加しない。
- 起動後の追記、新規セッション、新規日付 / プロジェクトディレクトリ、および
  更新時刻が固定された追記を実ファイルの監視で検証する。実ログは読み取り専用を維持する。

## 読める引出線注記（v2.27）

- 中央の発話は通常最大4行・幅200px、hover / 選択中は最大6行・幅220pxとする。
  画面が狭い場合は収まる幅へ縮める。文字サイズ・色・背景なしの引出線表現は維持する。
- `lastMessage` の既存60文字抜粋を維持し、引出線用に `lastMessageText`（最大320文字）を
  公開する。空白の正規化・メッセージ種別・時刻・重複抑制を共有し、古いsnapshotは
  `lastMessage` にフォールバックする。読取範囲や外部通信は増やさない。
- 自動注記は最大4エージェント。同じエージェントの発話と作業情報は1本にまとめ、作業情報は
  淡い補助1行とする。明示的なエージェント待機、進行中の計画ステップ、子への割当作業、
  直近45秒のコマンド結果の優先順で、取得済みデータがある場合だけ表示する。
  待機・計画・割当は稼働中の対象に限る。コマンド成功をテスト全体の成功と解釈しない。
- 最新の発話はセッションが表示対象である間、次の発話に置き換わるまで自動注記として残す。
  起動・再接続時も経過時間やthinking / waitingによらず表示し、10 / 18 / 45秒の消滅と
  起動時20秒制限を廃止する。発話が未取得の稼働中ツールは、観測したツール名と対象を表示する。
  新着優先と最大4件・空き領域の制約を維持し、画面内に収まらなかった候補も保持する。
  hover / 選択中の発話は自動注記と統合し、ブランチ・モデル・稼働時間の既存注記も維持する。
- 注記は他の注記、orb、ネームプレートを避け、画面内に収まる候補がない場合は表示を控える。
  一度選んだ方向を優先し、表示中に方向を切り替えて跳ね回らせない。画面サイズや内容の変更時は
  配置を再評価する。注記のためにエージェント自体を再配置しない。
- 30fps、hidden停止、reduced-motion、textContent / fillText、ログ読み取り専用を維持する。
  長文、複数エージェント、密集、画面端、選択、起動時の古い発話、無更新時の継続表示、退場を
  合成データで検証し、実ログでも確認する。
  本節はv2.13 / v2.18の2行・2件制限を更新する。
...
UI が観測したツール名から `tool` の表現を描き分ける。v2 の一律の継続波紋を次のように更新する。

| 作業 | 表現 | 対応するツール名 |
|---|---|---|
| 読む・調べる | 周辺から内側へ、少数の光点がゆっくり直線的に集まる | Read / Grep / Glob / WebFetch / WebSearch |
| 書く・編集する | orb の縁から外側へ短い光跡が伸びて消える | Edit / Write / NotebookEdit / apply_patch |
| コマンド実行 | 従来の 2.5 秒ごとの外向き波紋 | Bash / shell / exec_command |
| 未知・曖昧なツール | 従来の汎用波紋 | 上記以外・名前欠損・exec ラッパー |
| thinking | 呼吸を主とし、従来の旋回する微小光点を控えめに維持 | 既存状態 |
| waiting / idle | 従来の静かな待機・休息 | 既存状態 |

- 分類は大文字小文字を正規化した既知名の完全一致とし、Codex の `functions.apply_patch` /
  `functions.shell` / `functions.exec_command` も明示的に受理する。任意の名前空間・部分一致・
  `activityDetail` からの推測は行わない。`exec` / `functions.exec` は汎用表現とする。
  shell 内の読み取り・編集・テストの意味までは推定しない。
- 読む・書く間は継続波紋を置き換える。`lastActivity` 更新時の単発波紋・イベントポップ・
  ツール名と対象・マイクロアイコン・注記は維持する。新しい効果は orb の近傍に収め、
  衛星の縮尺にも合わせる。種類の切り替えは発光量の expLerp でつなぐ。
- 委任は既存の親子関係と子の実稼働に基づいて強調する。Codex satellite の
  `thinking` / `tool` 中は既存の関係線と流れる光点を緩やかに増光し、待機・退場で減光する。
  Claude spark は `running` の既存の周回光点と軌跡を強める。親の状態は変更せず、
  新しい関係線や通信量の推定は追加しない。
- 位置・親子構造・既存情報レイヤ・色系統を維持する。振動・高速点滅・オーバーレイは追加しない。
  効果は既存の 30fps / hidden 停止に従い、reduced-motion では止めて既存の文字・静的表現を保つ。
  本節の少数の局所効果を v2.3 のアニメーション追加制限に対する追加仕様とする。
- 分類・未知名・状態優先・遷移・reduced-motion を合成データで検証する。
...
- 開発用の撮影スクリプトは、一時ディレクトリにゼロから生成した架空の Claude / Codex JSONL だけを
  既存サーバーへ渡す。両方の watcher root を必ず指定し、普段のログ・設定・ブラウザプロファイルは使わない。
- 架空の2プロジェクトで調査・編集・テスト・親子セッションの推移を再生する。画面と計器は既存の
  パーサ・状態計算・UI をそのまま使用し、通常アプリへデモ用の表示や数値を追加しない。
- Electron の撮影専用プロファイルと offscreen 描画を使い、アプリの内容だけを FFmpeg で
  無音の H.264 MP4 にする。同じ描画から README 用の PNG を保存する。
  外部通信は既存の loopback 制限に従う。撮影用コードと FFmpeg は配布アプリへ同梱しない。
