...
2. **一回きり波紋**: tool の反復リング（1.25 秒・+38px・線幅 0.8）とテンポで差別化した単発リング。
   持続 ~2.8 秒・+~100px までゆっくり拡大・線幅 ~1.4・不透明度 ~0.3 から ease-out で減衰。
   色は既存どおり source コア色。「稼働のビート」ではなく「文末の句点」として読める遅さにする
3. **ハローの一呼吸**: 遷移観測から ~2.2 秒かけ、ハロー半径倍率を 1.0 → ~1.22 → 1.0 の単発の
   山なり（sine 半周期）で一度だけ膨らませ、輝度は既存 lerp で waiting 定常（0.58）へ着地させる。
   thinking の反復膨縮とは排他（waiting で反復呼吸をしない現状は維持）
4. **同時発火上限**: 同一 snapshot 適用で演出を発火できるのは最大 8 entity。超過分は演出なし
   （既存の輝度遷移のみ）
5. **reduced-motion**: 波紋・一呼吸とも無効化し、既存どおり輝度 lerp のみで表現する
