...
## 入力待ち遷移の演出（v2.19 — waiting 入りの一回きり波紋と一呼吸）

> tool には反復波紋、thinking には呼吸があるが、「ターンを終えて入力待ちに入った」瞬間は
> 輝度が静かに落ちるだけで見えない。眺めている人が周辺視野で「あ、ここはひと区切りついたな」と
> 気づける一回きりの光の演出を足す。ただしこれは完了通知ではない: 永続バッジ・点滅・文字は使わず、
> 遷移の瞬間だけ光で語り、以後は既存の waiting → idle の時間設計に任せる。

UI 側（ui/office.js のみ。コア・配信・UI テキストの変更なし）:

1. **トリガ**: Store の snapshot diff で、既存 entity の status が `waiting` 以外から `waiting` へ
   変わった瞬間に 1 回だけ発火する。新規 entity 作成時（起動直後・表示ウィンドウ入り）は発火しない。
   退場中（leaving）の entity も発火しない。対象は通常 orb と satellite（spark は session ではない
   ため対象外）。同一 entity の再発火は、一度 `waiting` 以外の status を観測した後の再遷移のみ
   - 観測は snapshot 依存であり「完了の瞬間」より遅れてよい。Codex は `task_complete` による
     状態変化 broadcast（~1 秒デバウンス）、Claude は 10 秒閾値経過後の次の snapshot
     （最大 15 秒のハートビート）で観測される
2. **二連の波紋**: tool の反復リング（1.25 秒・+38px・線幅 0.8）と本数・テンポ・明滅位置で
   差別化した 2 本のリングを ~0.5 秒差で放つ。各リングは持続 ~2.8 秒・+~100px までゆっくり拡大・
   線幅 ~2.0。不透明度は出現 0 → 進行中盤でピーク ~0.5 → 終端 0 の山なりエンベロープとし、
   ハローの外（orb 半径 +50〜70px 帯）に達した頃に最も明るくする（orb 密着時にピークを置くと
   ハローに埋もれて知覚できないため）。色は既存どおり source コア色。
   3 本以上は tool の反復リングと誤読されるため用いない
3. **ハローの一呼吸と遅めの減光**: 遷移観測から ~2.2 秒かけ、ハロー半径倍率を 1.0 → ~1.22 → 1.0 の
   単発の山なり（sine 半周期）で一度だけ膨らませる。輝度は遷移と同時に waiting 定常（0.58）へ
   落とさず、一呼吸の間は遷移時点の輝度を保ってから既存 lerp で沈む（減光との相殺で演出が
   打ち消されるのを防ぎ、「一度ふわっと光ってから休む」順序を作る）。
   thinking の反復膨縮とは排他（waiting で反復呼吸をしない現状は維持）
4. **同時発火上限**: 同一 snapshot 適用で演出を発火できるのは最大 8 entity。超過分は演出なし
   （既存の輝度遷移のみ）
5. **reduced-motion**: 波紋・一呼吸・減光の保持とも無効化し、既存どおり輝度 lerp のみで表現する

制約: 常設要素・文字・点滅・新色の追加なし。既存の波紋プリミティブ・描画順（Renderer の波紋レイヤ）・
30fps キャップ・`document.hidden` 停止・expLerp の構造を変えない。
