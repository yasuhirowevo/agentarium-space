...
![架空の2つのプロジェクトとセッションを表示したAgentarium Space](docs/media/agentarium-space-demo.png)
...
- `AGENTARIUM_WINDOW_MIN` — 活動情報の保持ウィンドウ（分、デフォルト 60）。非稼働セッションの表示は最長 15 分
...
- Claude Code のバックグラウンド sub-agent は起動・完了通知で追跡します。完了通知がない場合は、
  セッションが表示ウィンドウから外れるまで稼働中として保持します。大きなログの初回読込では、
  末尾の読取範囲に起動情報がある子だけを追跡します
- ログからコンテキスト上限を取得できない場合は、割合のリングを表示せず使用トークン数を表示します
